...
const CustomCardJob = ({ jobLogo, companyLogo, companyName, jobTitle, salary, jobDesc, className }) => {
   const notify = () => {
      const credentials = JSON.parse(localStorage.getItem("credentials"));
      if (credentials && credentials.isLoggedIn == 1) {
         customToast(`Applied at ${companyName}`, TOAST_TYPE.SUCCESS);
      } else {
         customToast("Please Sign In first", TOAST_TYPE.ERROR);
      }
   };
   ...
   return (
      ...
      >
         <img className="w-contain h-40 border-b border-gray-700" src={jobLogo.src} alt="role-image" />
         <div className=" p-4">
            <div className="grid grid-cols-[1fr_3fr]">
               <img src={companyLogo.src} alt="company-icon" className="rounded-full border border-black " />
               <div className="font-bold text-2xl flex items-center justify-center">{companyName}</div>
            </div>
            ...
            <p className=" text-gray-600 text-base text-justify line-clamp-2 ">{jobDesc}</p>
            <Button
